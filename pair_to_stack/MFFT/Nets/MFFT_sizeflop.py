    ...
    def __init__(self, dim_in, dim_out, kernel_size, padding, stride, scale_factor, bias=True):
        ...
        self.net = nn.Sequential(
            nn.Conv2d(
                ...
                groups=dim_in, stride=stride, dilation=scale_factor, bias=bias
            ),
    ...
    def __init__(self, dim, proj_kernel, kv_proj_stride, heads=8, dim_head=64, dropout=0.):
        ...
        for i in range(len(scale_factor)):
            self.Multi_scale_Token_Embeding.append(nn.ModuleList([
                CS_DepthWiseConv2d(
                    dim, inner_dim, proj_kernel, padding=scale_factor[i],
                    stride=1, scale_factor=scale_factor[i], bias=False
                ),
                CS_DepthWiseConv2d(
                    dim, inner_dim * 2, proj_kernel, padding=scale_factor[i],
                    stride=kv_proj_stride, scale_factor=scale_factor[i], bias=False
                ),
            ]))
...
class Cross_Scale_Visual_Attention_Module(nn.Module):
    def __init__(self, input_dim=32, dropout=0.):
        ...
        self.CvT = nn.Sequential(
            ...
            CS_LayerNorm(48),
            CS_Transformer(dim=48, proj_kernel=3, kv_proj_stride=2, heads=1, depth=1, mlp_mult=2, dropout=dropout),

            nn.Conv2d(48, 64, kernel_size=3, stride=2, padding=1),
            CS_LayerNorm(64),
            CS_Transformer(dim=64, proj_kernel=3, kv_proj_stride=2, heads=2, depth=1, mlp_mult=4, dropout=dropout),
        )
        self.relu = nn.ReLU(inplace=True)
...
class CD_Attention(nn.Module):
    def __init__(self, dim, proj_kernel, kv_proj_stride, heads=8, dim_head=64, dropout=0.):
        ...
        self.to_kv = CD_DepthWiseConv2d(dim, inner_dim * 2, proj_kernel, padding=padding, stride=kv_proj_stride, bias=False)
...
class Cross_Domian_Constrains(nn.Module):
    def __init__(self, dropout=0.):
        ...
        self.cdit = CD_Transformer(dim=48, proj_kernel=3, kv_proj_stride=4, heads=1, depth=1, mlp_mult=2, dropout=dropout)
...
class MFFT(nn.Module):
    ...
    def forward(self, A, B):
        ...
        fused_feat = self.mixer(concatenation)

        FG1 = self.csva_g_1(fused_feat)
        FD1 = self.csva_d_1(fused_feat)
...
if __name__ == '__main__':
    ...
    test_tensor_A = torch.randn((1, 3, H, W)).to(device)
    test_tensor_B = torch.randn((1, 3, H, W)).to(device)

    model = MFFT().to(device).eval()
    ...
    macs, params = profile(
        model,
        inputs=(test_tensor_A, test_tensor_B),
        verbose=False
    )
    ...
    macs_str, params_str = clever_format([macs, params], "%.4f")
    flops_str = clever_format([flops], "%.4f")[0]

    print(model)
    print("\n" + "=" * 50)
    print(f"Input size:         2 x (1, 3, {H}, {W})")
    print(f"Total Params:       {num_params}")
    print(f"Params (M):         {num_params / 1e6:.6f}")
    print(f"Trainable Params(M):{num_trainable_params / 1e6:.6f}")
    ...
    print("\nFor paper table:")
    print(f"Params(M): {num_params / 1e6:.3f}")
