import numpy as np
import torch
from torchvision import transforms as T
from skimage.morphology import remove_small_objects, binary_opening, binary_closing
from Utilities.CUDA_Check import GPUorCPU

DEVICE = GPUorCPU.DEVICE


def Binarization(img_tensor):

    return torch.where(img_tensor > 0.5, 1., 0.)


def RemoveSmallArea(img_tensor, size=None, threshold=0.001):

    if size is None:
        _, _, H, W = img_tensor.shape
        size = threshold * H * W

    img_array = img_tensor.detach().cpu().numpy().astype(bool)

    # 第一次去除小区域
    tmp_image1 = remove_small_objects(img_array, size)

    # 反相：前景<->背景，再做一次小区域去除
    tmp_image2 = (1 - tmp_image1).astype(bool)  # 这里把 np.bool 改成 bool
    tmp_image3 = remove_small_objects(tmp_image2, size)

    # 再反相回来
    tmp_image4 = 1 - tmp_image3
    tmp_image4 = tmp_image4.astype(np.float32)

    if isinstance(img_tensor, torch.Tensor):
        tmp_image4 = torch.from_numpy(tmp_image4).to(img_tensor.device)

    return tmp_image4